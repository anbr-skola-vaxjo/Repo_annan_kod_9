    ...
    public class Map
    {
        ...
        private void Add_keys(){
            for(int i = 0 ; i < Game1.nr_staring_keys ; i++){
                ...
                bool spawn_key = true;
                foreach(Item key in Game1.Keys_on_map){
                    if(key.MapP == new Vector2(X,Y)){
                        spawn_key = false;
                    }
                }
                if(Game1._Map.MapList[X,Y].Type == 0 && spawn_key){
                    Item I = new Item(Game1._Item_Types[0],new Vector2(X+0.5f,Y+0.5f));
                    Game1.Closest_key = I;
                    Game1.Keys_on_map.Add(I);
        ...
        public void Remove_keys(){
            foreach(Item i in Game1.Keys_on_map){
                _Entities.Remove(i._Entety);
            }
            Game1.Keys_on_map.Clear();
            Add_doors();
        }

        private void Add_doors(){
            if(Game1.Location == 2){
                for(int i = 0 ; i < Game1.nr_staring_keys ; i++){
                    int X = rd.Next(1,MapList.GetLength(0)-2);
                    int Y = rd.Next(1,MapList.GetLength(1)-2);
                    if(MapList[X,Y].Type == 1){
                        int h = 0;
                    
                        for(int iW = -1; iW <= 1 ; iW++){
                            for(int iH = -1; iH <= 1 ; iH++){
                                if(MapList[X+iW,Y+iH].Type == 0){
                                    h++;
                                }
